...
public class Scraper {
    ...
    ArrayList<InspectionObject> inspections = new ArrayList<>();
    URL url;
    InputStream is = null;
    BufferedReader br;
    BufferedWriter bw;
...
//    }
    ...
    public void removeHTML(Elements rows){
        ...
        for(Element row : rows){
            inspections.add(new InspectionObject());
...
//        scraper.removeHTML(scraper.getRows(scraper.getTableElement(scraper.fileToDocument(scraper.file))));

        try {
            scraper.removeHTML(scraper.getRows(scraper.getTableElement(scraper.fileToDocument(scraper.getDocument()))));
        } catch (IOException e) {
            e.printStackTrace();
        }
        System.out.println("Total: " + scraper.inspections.size()+"\n");
        System.out.println(scraper.inspections.get(0).getAll());
        System.out.println(scraper.inspections.get(10).getAll());
        System.out.println(scraper.inspections.get(100).getAll());
        System.out.println(scraper.inspections.get(1000).getAll());
        System.out.println(scraper.inspections.get(10000).getAll());
        System.out.println(scraper.inspections.get(59108).getAll());
    }
}
